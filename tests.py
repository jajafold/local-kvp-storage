...
class StorageTests(unittest.TestCase):
    def setUp(self) -> None:
        self.storage = Storage('repository_for_tests')
    ...
    def test_overloading_rehashing(self):
        complicated_key = SimpleClass()
        self.storage = Storage(capacity=2, repository_name='repository_for_tests')
        self.storage["1"] = 1
        self.storage["2"] = 2
    ...
    def test_collision(self):
        self.storage = Storage(repository_name='repository_for_tests', capacity=3)
        key_one = 3
        key_two = 6
        # buckets are equal
    ...
    def test_free(self):
        self.storage = Storage(repository_name='repository_for_tests')
        complicated_key = SimpleClass()
